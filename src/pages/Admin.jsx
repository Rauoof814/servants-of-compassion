...
export default function Admin() {
    ...
    async function loadAll() {
        ...
        try {
            ...
            if (!bl.error && bl.data) setPosts(bl.data);
            if (!pa.error && pa.data) setPartners(pa.data);
            if (!pr.error && pr.data) setPress(pr.data);
            if (!re.error && re.data) setResources(re.data);
            ...
            if (!homeRow.error && homeRow.data) {
                const row = homeRow.data;
                setHomeContent({
                    id: row.id ?? 1,
                    heroTitle: row.heroTitle ?? "",
                    heroTitleUk: row.heroTitleUk ?? "",
                    heroSubtitle: row.heroSubtitle ?? "",
                    heroSubtitleUk: row.heroSubtitleUk ?? "",
                    stats: {
                        ambulances: row?.stats?.ambulances ?? 0,
                        kits: row?.stats?.kits ?? 0,
                        volunteers: row?.stats?.volunteers ?? 0,
                        communities: row?.stats?.communities ?? 0,
                    },
                    impactStatement: row.impactStatement ?? "",
                    impactStatementUk: row.impactStatementUk ?? "",
    ...
    React.useEffect(() => { if (user) loadAll(); }, [user]);

    // ---------- edit handlers ----------
    const startEdit = (type, item) => { setEditingType(type); setEditingItem(item); };
    const cancelEdit = () => { setEditingType(null); setEditingItem(null); };
    ...
    const handleUpdate = async (e, type, id) => {
        ...
        try {
            ...
            for (let [key, value] of f.entries()) if (value !== null && value !== "") updates[key] = value;
            if (image_url) updates.image_url = image_url;
        ...
        <main className="pt-28 pb-24">
            <div className="container grid lg:grid-cols-[240px,1fr] gap-6">
                <aside className="sticky top-[84px] self-start rounded-2xl border border-white/10 bg-white/5 backdrop-blur p-3 h-fit">
                    ...
                    <div className="mt-3 pt-3 border-t border-white/10">
                        <button className="btn w-full" onClick={() => supabase.auth.signOut()}>Logout</button>
                    </div>
                </aside>
                ...
                <section className="grid gap-6">
                        ...
                        <div className="grid lg:grid-cols-2 gap-8">
                                        ...
                                        <div>
                                            ...
                                            <div className="grid gap-3">
                                                ...
                                                <button
                                                    className="btn"
                                                    onClick={() =>
                                                        setHero({ ...hero, slides: [...(hero.slides || []), { src: "" }] })
                                                    }
                                                >
                                                    + Add Slide
                                                </button>
                            ...
                            <div className="card p-6">
                                ...
                                <div className="grid gap-4">
                                    ...
                                    <div className="h4 mt-6">Impact Statistics</div>
                                    <div className="grid md:grid-cols-2 gap-4">
                                        {["ambulances", "kits", "volunteers", "communities"].map((k) => (
                                            <div key={k}>
                                                <label className="label">
                                                    {k === "ambulances" && "Ambulances Delivered"}
                                                    {k === "kits" && "Medical Kits Provided"}
                                                    {k === "volunteers" && "Active Volunteers"}
                                                    {k === "communities" && "Communities Served"}
                                                </label>
                                                <input
                                                    className="input"
                                                    type="number"
                                                    value={homeContent.stats?.[k] ?? 0}
                                                    onChange={(e) =>
                                                        setHomeContent({
                                                            ...homeContent,
                                                            stats: { ...homeContent.stats, [k]: parseInt(e.target.value) || 0 },
                                                        })
                                                    }
                                                />
                                            </div>
                        ...
                        <div className="card p-6">
                            ...
                            >
                                <input className="input" name="title" placeholder="Title (EN)" required
                                    defaultValue={editingType === "events" ? editingItem.title : ""} />
                                <input className="input" name="title_uk" placeholder="Title (UK) (optional)"
                                    defaultValue={editingType === "events" ? editingItem.title_uk || "" : ""} />
                                <input className="input" name="date" type="date" required
                                    defaultValue={editingType === "events" ? editingItem.date : ""} />
                                <input className="input" name="location" placeholder="Location"
                                    defaultValue={editingType === "events" ? editingItem.location || "" : ""} />
                                <input className="input" name="provides" placeholder="What this event provides"
                                    defaultValue={editingType === "events" ? editingItem.provides || "" : ""} />
                                <input className="input" name="image" type="file" accept="image/*" />
                                <input className="input md:col-span-2" name="video_url" placeholder="(Optional) YouTube/Vimeo/Instagram/.mp4"
                                    defaultValue={editingType === "events" ? editingItem.video_url || "" : ""} />
                                <textarea className="input md:col-span-2" name="description" placeholder="Description"
                                    defaultValue={editingType === "events" ? editingItem.description || "" : ""} />
                                <button className="btn btn-yellow md:col-span-2">
                                    {editingType === "events" ? "Update Event" : "Add Event"}
                                </button>
                                {editingType === "events" && (
                                    <button className="btn md:col-span-2" type="button" onClick={cancelEdit}>Cancel</button>
                                )}
                            </form>
                            ...
                            <div className="grid md:grid-cols-3 gap-3 mt-2">
                                {events.map((ev) => (
                                    <div key={ev.id} className="card p-3 grid gap-2">
                                        ...
                                        {ev.image_url && <img src={ev.image_url} alt="" className="rounded-xl border border-white/10" />}
                                        <div className="flex gap-2">
                    ...
                    {tab === "blog" && (
                        <div className="card p-6">
                            ...
                            >
                                <input className="input" name="title" placeholder="Title (EN)" required
                                    defaultValue={editingType === "blog" ? editingItem.title : ""} />
                                <input className="input" name="title_uk" placeholder="Title (UK) (optional)"
                                    defaultValue={editingType === "blog" ? editingItem.title_uk || "" : ""} />
                                <input className="input" name="date" type="date"
                                    defaultValue={editingType === "blog" ? editingItem.date : ""} />
                                <input className="input" name="author" placeholder="Author"
                                    defaultValue={editingType === "blog" ? editingItem.author || "" : ""} />
                                <input className="input" type="file" name="image" accept="image/*" />
                                <input className="input md:col-span-2" name="video_url" placeholder="(Optional) YouTube/Vimeo/.mp4/Instagram"
                                    defaultValue={editingType === "blog" ? editingItem.video_url || "" : ""} />
                                <textarea className="input md:col-span-2" rows={6} name="content" placeholder="Article content"
                                    defaultValue={editingType === "blog" ? editingItem.content || "" : ""} />
                                <button className="btn btn-yellow md:col-span-2">
                                    {editingType === "blog" ? "Update Post" : "Publish"}
                                </button>
                                {editingType === "blog" && (
                            ...
                            <div className="grid md:grid-cols-3 gap-3 mt-2">
                                {posts.map((p) => (
                                    <div key={p.id} className="card p-3 grid gap-2">
                                        ...
                                        {p.image_url && <img src={p.image_url} alt="" className="rounded-xl border border-white/10" />}
                                        <div className="flex gap-2">
                    ...
                    {tab === "media" && (
                        <div className="card p-6">
                            ...
                            <div className="h3 mt-6">Current Media</div>
                            <div className="grid md:grid-cols-3 gap-3 mt-2">
                                {(resources || []).filter((r) => r.kind === "video" || r.video_url).map((r) => (
                                    <div key={r.id} className="card p-3 grid gap-2">
                                        <b>{r.title || r.video_url}</b>
                                        {r.file_url && <img src={r.file_url} alt="" className="rounded-xl border border-white/10" />}
                                        <div className="flex gap-2">
                        ...
                        <div className="card p-6">
                            ...
                            >
                                <input className="input" name="name" placeholder="Name" required
                                    defaultValue={editingType === "partners" ? editingItem.name : ""} />
                                <input className="input" name="url" placeholder="Website"
                                    defaultValue={editingType === "partners" ? editingItem.url || "" : ""} />
                                <input className="input" type="file" name="logo" accept="image/*" />
                                <button className="btn btn-yellow">
                                    {editingType === "partners" ? "Update Partner" : "Add Partner"}
                                </button>
                            ...
                            <div className="grid md:grid-cols-3 gap-3 mt-2">
                                {partners.map((p) => (
                                    <div key={p.id} className="card p-3 grid gap-2">
                                        <b>{p.name}</b>
                                        {p.logo_url && <img src={p.logo_url} alt="" className="rounded-xl border border-white/10" />}
                                        <div className="flex gap-2">
                        ...
                        <div className="card p-6">
                            ...
                            >
                                <input className="input" name="title" placeholder="Title" required
                                    defaultValue={editingType === "press" ? editingItem.title : ""} />
                                <input className="input" name="date" type="date" required
                                    defaultValue={editingType === "press" ? editingItem.date : ""} />
                                <input className="input" name="outlet" placeholder="Outlet"
                                    defaultValue={editingType === "press" ? editingItem.outlet || "" : ""} />
                                <input className="input" name="url" placeholder="Link URL"
                                    defaultValue={editingType === "press" ? editingItem.url || "" : ""} />
                                <textarea className="input md:col-span-2" name="excerpt" placeholder="Short excerpt"
                                    defaultValue={editingType === "press" ? editingItem.excerpt || "" : ""} />
                                <button className="btn btn-yellow md:col-span-2">
                                    {editingType === "press" ? "Update Press" : "Add Press"}
                                </button>
                                {editingType === "press" && (
                    ...
                    {tab === "resources" && (
                        <div className="card p-6">
                            ...
                            >
                                <input className="input" name="title" placeholder="Title" required
                                    defaultValue={editingType === "resources" ? editingItem.title : ""} />
                                <input className="input" name="url" placeholder="Link URL (optional)"
                                    defaultValue={editingType === "resources" ? editingItem.url || "" : ""} />
                                <input className="input" type="file" name="file" />
                                <textarea className="input md:col-span-2" name="description" placeholder="Description"
                                    defaultValue={editingType === "resources" ? editingItem.description || "" : ""} />
                                <button className="btn btn-yellow md:col-span-2">
                                    {editingType === "resources" ? "Update Resource" : "Add Resource"}
                                </button>
                                {editingType === "resources" && (
                            ...
                            <div className="h3 mt-6">Current Resources</div>
                            <div className="grid md:grid-cols-3 gap-3 mt-2">
                                {(resources || []).filter((r) => r.kind === "doc").map((r) => (
                                    <div key={r.id} className="card p-3 grid gap-2">
                                        <b>{r.title}</b>
                    ...
                    {tab === "inbox" && (
                        <div className="card p-6">
                            <div className="h3">Inbox</div>
                            <div className="opacity-70">
                                Connect your forms to Supabase tables (contacts, volunteers) and list them here.
                            </div>
                        </div>
                    )}
                    ...
                    {tab === "i18n" && (
                        <div className="card p-6 grid gap-2">
                            <div className="h3">Texts (EN/UK)</div>
                            <label className="label">About long (EN)</label>
                            <textarea className="input" value={content.aboutLongEn || ""}
                                onChange={(e) => setContent({ ...content, aboutLongEn: e.target.value })} />
                            <label className="label">About long (UK)</label>
                            <textarea className="input" value={content.aboutLongUk || ""}
                                onChange={(e) => setContent({ ...content, aboutLongUk: e.target.value })} />
                            <label className="label">Mission long (EN)</label>
                            <textarea className="input" value={content.missionLongEn || ""}
                                onChange={(e) => setContent({ ...content, missionLongEn: e.target.value })} />
                            <label className="label">Mission long (UK)</label>
                            <textarea className="input" value={content.missionLongUk || ""}
                                onChange={(e) => setContent({ ...content, missionLongUk: e.target.value })} />
                            <button
                                className="btn btn-yellow mt-2"
                                onClick={async () => {
                                    try {
                    ...
                    {/* -------- Brand & Social (with upload) -------- */}
                    {tab === "brand" && (
                        <div className="card p-6 grid md:grid-cols-2 gap-3">
                            <label className="label">Name</label>
                            <input className="input" value={brand.name}
                                onChange={(e) => setBrand({ ...brand, name: e.target.value })} />

                            <label className="label">Logo URL</label>
                            <input className="input" value={brand.logo}
                                onChange={(e) => setBrand({ ...brand, logo: e.target.value })} />
                            ...
                            {brand.logo && (
                                <div className="md:col-span-2">
                                    <div className="text-xs opacity-70 mb-1">Logo preview</div>
                                    <img src={brand.logo} alt="logo" className="h-12 rounded border border-white/10" />
                                </div>
                            )}

                            <label className="label">Address</label>
                            <input className="input" value={brand.address}
                                onChange={(e) => setBrand({ ...brand, address: e.target.value })} />

                            <label className="label">Email</label>
                            <input className="input" value={brand.email}
                                onChange={(e) => setBrand({ ...brand, email: e.target.value })} />

                            <label className="label">Phone</label>
                            <input className="input" value={brand.phone}
                                onChange={(e) => setBrand({ ...brand, phone: e.target.value })} />

                            <label className="label">Facebook</label>
                            <input className="input" value={brand.social.facebook || ""}
                                onChange={(e) => setBrand({ ...brand, social: { ...brand.social, facebook: e.target.value } })} />

                            <label className="label">Instagram</label>
                            <input className="input" value={brand.social.instagram || ""}
                                onChange={(e) => setBrand({ ...brand, social: { ...brand.social, instagram: e.target.value } })} />

                            <label className="label">Twitter / X</label>
                            <input className="input" value={brand.social.twitter || ""}
